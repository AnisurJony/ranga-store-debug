...
const showProducts = (products) => {
  ...
  for (const product of allProducts) {
    const title = product.title.slice(0, 20); //--------------- I have done it for avoiding long title that make the title very poor in decorative...................//
    ...
    <img class="product-image" src=${image}></img>
      ...
      <p> Rating:${product.rating.rate} satisfied:${product.rating.count}</p>
...
//--------------------------------function for number of product add to the card----------------------//
let count = 0;
const addToCart = (id, price) => {
  ...
  updateTaxAndCharge();
  document.getElementById("total-Products").innerText = count; // $ sign remove before count at the HTML file from respective zone
};

// ---------------------------------get the product price  for further calculation------------------------------//
...
const updatePrice = (id, value) => {
  ...
  document.getElementById(id).innerText = total.toFixed(2);  // here I change code for geting exect price from the data as it displayed in the ui 
};
...
const updateTaxAndCharge = () => {
  ...
  if (priceConverted > 200) {
    ...
    const tax = priceConverted * 0.2;   //  I have added extra line of code for taking two digit after period in fruction(1)
    setInnerText("total-tax", tax.toFixed(2)); //---------- above line for this
  }
  if (priceConverted > 400) {
    setInnerText("delivery-charge", 50);
    const tax = priceConverted * 0.3;  //  I have added extra line of code for taking two digit after period in fruction(2)
    setInnerText("total-tax", tax.toFixed(2)); //----------above line for this
  }
  if (priceConverted > 500) {
    setInnerText("delivery-charge", 60);
    const tax = priceConverted * 0.4;  //  I have added extra line of code for taking two digit after period in fruction(3)
    setInnerText("total-tax", tax.toFixed(2)); //-------------above line for this
  }

  updateTotal(); // the function has been call here that wasn't in previous code
};
...
const updateTotal = () => {
  ...
  document.getElementById("total").innerText = grandTotal.toFixed(2); // has updated code here for getteing two digit after period in total calculation.
};

//------------------------------- call the Load API function--------------------------------//
loadProducts();
